...
ggplot(mdat1, aes(x =age, fill = age, group = age)) +
  geom_bar(position = 'dodge') + scale_fill_brewer(palette = "Reds")+ labs(title="Respondent Age Groups", x = "Age Group", y = "Total Number of Respondents")
...
x <- mdat2$age
plot(x, mdat2$discuss.supervisor, col="blue")
fits <- fitted(mod1)
...
mdat2 <- mdat1 %>% filter(discuss.supervisor !="")
table(mdat2$discuss.supervisor)
mdat2$discuss.supervisor <- factor(mdat2$discuss.supervisor, levels = c("Yes","No","Maybe"))
...
mdat4$prev.anonymity.protected <- relevel(mdat4$prev.anonymity.protected, ref="Yes, always")
...
mod1 <- multinom(discuss.supervisor~age + organization.size.large + mental.health.coverage + prev.anonymity.protected, data=mdat4, family = "multinom")

mod2 <- multinom(discuss.supervisor~age + mental.health.coverage + prev.anonymity.protected, data=mdat4, family = "multinom")

preds <- predict(mod2)

summary(mod1)

table(mdat4$discuss.supervisor,preds)
...
mod_fit <- train(discuss.supervisor~age + organization.size.large + mental.health.coverage + prev.anonymity.protected,  data=mdat4, method="multinom",
...
mapCountryData(spdf, nameColumnToPlot="freq", catMethod="fixedWidth", mapTitle = "Survey Responses Distribution - World")
...
spplot(USAsp['value'], col.regions=c(brewer.pal(8,"Reds"),"#700000","#600000","#680000","#580000","#500000","#480000","#300000","#200000"))
...
# Plot this without grouping age into Young and Senior
plot(dat$age,dat$discuss.supervisor, col=c("indianred2","#A80000","#600000"), ylab="Comfort level in discussion with Supervisor", xlab="Age of the Respondent",main="Comfort level with Age")

?brewer.pal
ggplot(dat, aes(x=discuss.supervisor, fill=age, group=age)) + geom_bar(position = 'dodge') + scale_fill_brewer(palette = "Set3")

mod <- multinom(dat$discuss.supervisor~dat$age, family="multinom")
...
percent$value[3] <- 41.12
percent$value[4] <- 39.8
...
ggplot() +
  ...
  geom_path(data = percent2, aes(x = value, y = methods, col=c("blue","red","blue","red","blue","red","blue","red")), size = 2) +
